...
legacy Next.js app was preserved under `legacy_nextjs_backup/` for reference. The
...
remove (never delete `legacy_nextjs_backup/` — user requires it kept).
...
app, check for `legacy_nextjs_backup/` first — the Streamlit app at the root is
...
committed. No functional code was affected. The `legacy_nextjs_backup/` folder
