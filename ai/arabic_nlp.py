...
# وُسِّع من 55 إلى 287 جذراً (تغطية أوسع لمفردات النصوص الدينية/القرآنية:
# الأسرة، الطبيعة، الجسد، الزمن، التجارة، الحرب، العبادات، الأخلاق...)
...
class SyntacticAnalyser:
    ...
    def _tag_token(self, raw: str) -> ArabicToken:
        ...
        # أداة مسبوقة بواو/فاء عطف قصيرة (مثل "وإن"→"وان"، "فلم"، "ولن")
        # كانت تُصنَّف أفعالاً بالخطأ: _strip_affixes يرفض نزع سابقة بحرف
        # واحد إن لم يتبقَّ ≥3 أحرف (لحماية جذور ثلاثية حقيقية مثل "كتب")،
        # فتبقى هذه الكلمة القصيرة كاملة وتُحلَّل خطأً كفعل ماضٍ من 3 أحرف.
        if len(clean) >= 3 and clean[0] in "وف" and clean[1:] in _PARTICLES:
            return ArabicToken(
                raw=raw, clean=clean, stripped=clean[1:], prefix=clean[0],
                pos="particle",
            )
...
class MorphologicalAnalyser:
    ...
    def _extract_root(self, word: str, pos: str) -> Tuple[str, float]:
        ...
        # حالة خاصة: اسم الجلالة "الله" أهم كلمة بالنصوص الدينية لهذا
