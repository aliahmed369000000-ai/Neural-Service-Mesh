...
# ── 🚀 أداء: استيراد torch كسول (lazy) ───────────────────────────────────
# torch (~1.5s عند الاستيراد) لا يُحمَّل إلا عند أول استخدام فعلي لمكونات MoE.
# typing annotations مؤجلة تلقائيًا (from __future__) فلا يُحتاج torch module-level.


def _ensure_torch():
    """يحمّل torch/nn/F مرة واحدة ويعيدها. singleton خفيف."""
    import torch as _t          # noqa: E401
    import torch.nn as _n       # noqa: E401
    import torch.nn.functional as _f  # noqa: E401
    return _t, _n, _f


_CLASSES_DEFINED = False  # 🚀 حماية إعادة تعريف الفئات


def _init_classes():
    """🚀 أداء: فئات MoE المعتمدة على torch تُعرَّف هنا عند أول استخدام فعلي —
    torch (~1.5s) لا يُحمَّل إلا عند الحاجة. typing annotations مؤجلة
    (from __future__ import annotations)."""
    global _CLASSES_DEFINED, torch, nn, F
    if _CLASSES_DEFINED:
        return
    torch, nn, F = _ensure_torch()
    global ExpertMeta, ExpertFFN, TopKRouter, ExpertGroup, HierarchicalMoE, train_step, build_default_moe, handle_moe_command, _self_test
...
# ── إعدادات MoE الافتراضية (module-level، لا torch مطلوب) ──────────────
...
# ── 🚀 proxy: أول وصول لأي مكوّن MoE يفعّل تعريف الفئات (lazy) ───────
_MOE_NAMES = (
    "ExpertMeta", "ExpertFFN", "TopKRouter", "ExpertGroup", "HierarchicalMoE",
    "train_step", "build_default_moe", "handle_moe_command", "_self_test",
)


# 🚀 proxies جاهزة module-level (لا حمولة torch): تُفعّل الفئات عند النداء الأول فقط
ExpertMeta = ExpertFFN = TopKRouter = ExpertGroup = HierarchicalMoE = None
train_step = build_default_moe = handle_moe_command = _self_test = None
def _moe_activate():
    """تفعيل تأجيلي: تعريف الفئات عند أول استخدام فعلي."""
    _init_classes()
def _moe_proxy(func_name):
    def _lazy(*a, **kw):
        _moe_activate()
        return globals()[func_name](*a, **kw)
    return _lazy
ExpertMeta = _moe_proxy("ExpertMeta")
ExpertFFN = _moe_proxy("ExpertFFN")
TopKRouter = _moe_proxy("TopKRouter")
ExpertGroup = _moe_proxy("ExpertGroup")
HierarchicalMoE = _moe_proxy("HierarchicalMoE")
train_step = _moe_proxy("train_step")
build_default_moe = _moe_proxy("build_default_moe")
handle_moe_command = _moe_proxy("handle_moe_command")
_self_test = _moe_proxy("_self_test")

def __getattr__(name):
    """module-level __getattr__: يفعّل تعريف الفئات عند أول وصول."""
    if name in _MOE_NAMES:
        _init_classes()
        return globals()[name]
    raise AttributeError(f"module 'ai.hierarchical_moe' has no attribute {name!r}")
# ── module-level constants (لا torch مطلوب) ─────────────────────────────
logger = logging.getLogger("HierarchicalMoE")
ROOT = Path(__file__).resolve().parent.parent
DEFAULT_SAVE_DIR: "Path" = ROOT / "artifacts" / "hierarchical_moe"


def _ensure_save_dir():
    """🚀 أداء: ينشئ مجلد الحفظ عند أول استخدام فعلي (بدون torch)."""
    DEFAULT_SAVE_DIR.mkdir(parents=True, exist_ok=True)


# فحص ذاتي سريع عند التشغيل المباشر (python3 ai/hierarchical_moe.py)
