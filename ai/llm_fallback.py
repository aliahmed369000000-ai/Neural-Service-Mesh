...
def humanize_fallback_error(raw_error: Optional[str]) -> str:
    """يحوّل FallbackResult.error/FableChapter.error الخام إلى رسالة عربية
    مبسّطة آمنة للعرض المباشر على المستخدم النهائي.

    المشكلة: عند فشل كل مزوّدي LLM الحيّين، generate()/generate_stream()
    يملآن هذا الحقل بسلسلة "تريد" خام مثل:
        "فشلت كل المزوّدين: ['groq:err(429 Too Many Requests...)',
         'cerebras:err(503 Service Unavailable...)', 'anthropic:err(...)']"
    وهذا نص تشخيصي داخلي (رموز HTTP، مقتطفات استثناءات مكتبات urllib)
    كان يُعرض كما هو لعدة صفحات مستخدم عادية (الترجمة، السرد الإبداعي)
    عبر st.caption("⚠️ ملاحظة تقنية: ...") — نفس فئة المشكلة الموثَّقة
    سابقاً مع تسريب خطأ DuckDuckGo الخام لحلقة self-healing في
    nsm_agent_core.py، لكن هنا في واجهة المستخدم مباشرة.

    لا يُستخدم لأي غرض تشخيصي/سجلّات (استخدم result.error/logger مباشرة
    هناك) — فقط لنص يُعرض على المستخدم النهائي دون تفاصيل تقنية خام.
    ترجع نصاً فارغاً إن لم يوجد خطأ أصلاً."""
    if not raw_error:
        return ""
    return (
        "تعذّر الوصول لأي نموذج ذكاء اصطناعي حيّ مؤقتاً "
        "(المزوّدون المتاحون غير مستجيبين أو استُنفدت حصتهم المجانية حالياً) "
        "— هذا رد احتياطي مبني على قاعدة المعرفة الداخلية، وقد يعمل التوليد "
        "الكامل عند إعادة المحاولة لاحقاً."
    )
