...
_CLUSTER_TO_CATEGORY: Dict[str, str] = {
    ...
    "tafsir": "tafsir", "تفسير": "tafsir", "quran": "tafsir", "قرآن": "tafsir", "قران": "tafsir", "ayah": "tafsir",
    ...
    # لغة
    "arabic": "arabic", "لغة": "arabic", "نحو": "arabic", "بلاغة": "arabic", "nahw": "arabic",
    # تخصصات
    "programming": "programming", "code": "programming", "coding": "programming", "برمجة": "programming",
    "sports": "sports", "رياضة": "sports", "football": "sports",
    "science": "science", "علوم": "science", "physics": "science",
    "math": "math", "رياضيات": "math", "رياضيات": "math",
    "medicine": "medicine", "طب": "medicine", "صحة": "medicine",
    "technology": "technology", "تقنية": "technology", "ai": "technology",
    "business": "business", "أعمال": "business", "تجارة": "business",
    "education": "education", "تعليم": "education", "تربية": "education",
    "history": "history", "تاريخ": "history",
    "media": "media", "إعلام": "media",
    "psychology": "psychology", "نفس": "psychology", "علم_نفس": "psychology",
    "law": "law", "قانون": "law",
...
_KEYWORD_TO_CATEGORY: List[Tuple[str, str]] = [
    ...
    (r"حديث|أحاديث|رواة|إسناد|صحيح\s*البخاري|مسلم|ترمذي", "hadith"),
    (r"عقيدة|توحيد|إيمان|أسماء\s*الله|قدر|شرك", "aqidah"),
    (r"سيرة|غزوة|صحابة|خلافة|نبوي|الهجرة", "seerah"),
    (r"نحو|صرف|بلاغة|إعراب|لغة\s*عربي|قواعد\s*اللغة", "arabic"),
    # تخصصات
    (r"برمج|برمجة|python|جافا|javascript|كود|خوارزمي|git|api|دالة|متغير|class\b|function", "programming"),
    (r"رياضة|كرة\s*قدم|مباراة|تدريب\s*رياضي|لياقة|أولمبي|بطن|جيم", "sports"),
    (r"فيزياء|كيمياء|أحياء|بيولوجيا|علوم\s*طبيعي|تجربة\s*علمي", "science"),
    (r"رياضيا|رياضيات|جبر|هندسة\s*مستوية|إحصاء|معادلة|مشتق|تكامل|matrix", "math"),
    (r"طب|مرض|علاج|دواء|صحة|تغذية|فيروس|مستشفى|أعراض", "medicine"),
    (r"ذكاء\s*اصطناعي|شبكات|حاسوب|سيرفر|أمن\s*سيبراني|هاردوير|تقنية", "technology"),
    (r"تسويق|مبيعات|تمويل|إدارة\s*أعمال|شركة|استثمار|ريادة|أرباح", "business"),
    (r"تعليم|مناهج|تدريس|طالب|مدرسة|جامعة|اختبار|منهج", "education"),
    (r"تاريخ|حضارة|دولة\s*عثمانية|حرب\s*عالمية|آثار|مؤرخ", "history"),
    (r"إعلام|صحافة|محتوى|سوشيال|منصة|يوتيوب|تويتر", "media"),
    (r"علم\s*نفس|قلق|اكتئاب|سلوك|شخصية|تحفيز", "psychology"),
    (r"قانون|محكمة|دعوى|تشريع|عقد\s*قانوني|محام", "law"),
]
