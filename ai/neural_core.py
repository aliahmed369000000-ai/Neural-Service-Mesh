...
DEFAULT_INPUT_DIM = 7          # ثابت — 7 ميزات مدخل (لا يتغير)
DEFAULT_HIDDEN_DIMS = [112, 32]  # الطبقة الأولى 112×7 (أوزان مدروسة من nnn_112.csv)  # طبقة أولى 108×7 (أوزان مدروسة) + طبقة ثانية 32×108 (Xavier)
...
_NEURAL_CORE_WEIGHTS = [
    # ══ 112 صف × 7 أعمدة — أوزان مدروسة من nnn_112.csv ══
    # البنية: Input(7) → L1(112×7, relu, مدروسة) → L2(عشوائية, relu) → L3(4, softmax)
    # الطبقتان L2 و L3 تتعلمان بالتدريب (Xavier init + backprop)
    ...
    [0.11  , 0.6   , 0.7   , 0.1   , 0.4   , 0.5   , 0.1   ],
    [0.18  , 0.8   , 0.1   , 0.6   , 0.4   , 0.1   , 0.11  ],
    [0.8   , 0.6   , 0.1   , 0.3   , 0.13  , 0.11  , 0.0   ],
    [0.14  , 0.0   , 0.1   , 0.3   , 0.4   , 0.0   , 0.1   ],
]
...
class NeuralCore:
    ...
    def __init__(
        ...
        if first_layer.W.shape == (112, 7):
            first_layer.W = np.array(_NEURAL_CORE_WEIGHTS, dtype=np.float64)
            first_layer.b = np.full(112, 0.6, dtype=np.float64)
