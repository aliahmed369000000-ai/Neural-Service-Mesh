...
_NEURAL_CORE_WEIGHTS = [
    ...
    # الصف 106:
    [0.3,  0.6,  0.1,  0.5,  0.2,  0.4,  0.7 ],
    # الصف 107:
    [0.5,  0.3,  0.2,  0.1,  0.4,  0.6,  0.3 ],
    # الصف 108 (المضاف):
...
class NeuralCore:
    ...
    def __init__(
        ...
        # البنية الافتراضية: 7 → [108×7 relu] → [32×108 relu] → [4×32 softmax]
        ...
        if first_layer.W.shape == (108, 7):
            first_layer.W = np.array(_NEURAL_CORE_WEIGHTS, dtype=np.float64)
            first_layer.b = np.full(108, 0.6, dtype=np.float64)
    ...
    def evolve_if_plateau(self) -> bool:
        ...
        n = last_layer.in_dim  # 32 في البنية الافتراضية
