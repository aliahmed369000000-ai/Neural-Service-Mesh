"""
Phase 8+ — Real Neural Weights (784 × 784 matrix, bias=0.6)
==========================================================
Provides a numpy-backed weight matrix for the RoutingEngine.
موحَّدة (v20) مع L1 المدروسة في ai/neural_core.py — المصفوفة الكاملة من
weights_784x784.csv (784 صف × 784 عمود، نفس مصدر بيانات المستخدم).

Architecture
------------
  NeuralWeightLayer
    ├── weights  : np.ndarray  shape=(784, 784)
    ...
    └── load(path)           → restore from .npy file (accepts any (N,784) shape)
...
Growth strategy
---------------
  Columns are ALWAYS 784 — fixed to the feature vector size (يطابق neural_core.py).
  Rows start at 784 (مصفوفة المستخدم الكاملة المدروسة، قابلة للنمو عبر DynamicWeightLayer).
"""
...
# ── Initial weight matrix (5 rows × 784 columns — مصفوفة المستخدم المدروسة) ──
_INITIAL_WEIGHTS_PATH = Path(__file__).parent / "models" / "initial_weights.npy"


def _load_initial_weights() -> "np.ndarray":
    """يحمّل الأوزان الابتدائية من ملف .npy — lazy loading، لا يُحمَّل عند الاستيراد.

    الملف: ai/models/initial_weights.npy (784 × 784، float64)
    مُولَّد مرة واحدة من المصفوفة الأصلية المضمّنة في الكود السابق.
    """
    if not _INITIAL_WEIGHTS_PATH.exists():
        raise FileNotFoundError(
            f"ملف الأوزان الابتدائية غير موجود: {_INITIAL_WEIGHTS_PATH} — "
            "تأكد من وجود ai/models/initial_weights.npy في المشروع."
        )
    return np.load(str(_INITIAL_WEIGHTS_PATH))



class NeuralWeightLayer:
    """
    Neural weight layer backed by a (784 × 784) numpy matrix with scalar bias.

    Design contract
    ---------------
    • COLUMNS are FIXED at 784 — input_dim = 7 دلالي + 777 TF-IDF hash.
    • ROWS are 784 (أوزان مدروسة من مصفوفة المستخدم 784×784 الكاملة).
    • bias is a scalar applied after the matrix multiply (default 0.6).

    Parameters
    ----------
    initial_weights : array-like, optional
        Seed weights with shape (N, 784).  Defaults to the 784-row matrix.
    bias : float
    ...
    COLS = 784  # input_dim = 784 (7 دلالي + 777 TF-IDF hash، يطابق neural_core.py)
    ...
    def __init__(
        ...
        if initial_weights is not None:
            ...
            if w.ndim != 2 or w.shape[1] != self.COLS:
                raise ValueError(
                    f"NeuralWeightLayer expects shape (N, {self.COLS}=784), got {w.shape}"
                )
            self.weights: np.ndarray = w.copy()
        else:
            self.weights = _load_initial_weights().astype(np.float64)
...
def get_default_layer(weights_path: str = "models/classifiers/routing_weights.npy") -> NeuralWeightLayer:
    ...
    Otherwise the 784-row initial matrix (مصفوفة المستخدم المدروسة الكاملة) is used.
    """
