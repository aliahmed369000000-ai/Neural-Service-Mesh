...
# لماذا لا يوجد محول Snapchat (المرحلة 3)

طُلب فحص Snapchat لإضافته كمحول جديد (نشر/مراقبة/رد). الفحص الفعلي
لوثائق Snap for Developers الحالية يُظهر عدم وجود مسار API علني يحقق
عقد `PlatformAdapter` (نشر منشور عام + مراقبة + رد) لأي حساب:

- **Marketing API**: حملات إعلانية مدفوعة فقط (استهداف، ميزانيات،
  creatives إعلانية) — ليس نشراً عضوياً لمحتوى، ولا صلة له بمراقبة/رد.
- **Snap Kit (Creative Kit)**: مشاركة محتوى **من تطبيق المستخدم إلى**
  Snapchat عبر واجهة مشاركة على جهاز المستخدم نفسه (client-side share
  sheet) — وليس استدعاء خادمي (server-to-server) يمكن لبوت تنفيذه نيابة
  عن حساب بلا تدخل يدوي من المستخدم في كل مرة.
- **Story Kit**: قراءة/تضمين قصص عامة موجودة أصلاً — لا نشر ولا رد.
- لا يوجد أي endpoint موثَّق لقراءة الرسائل/التعليقات الواردة أو الرد
  عليها لحساب أعمال عادي (خارج شراكات خاصة غير معلنة).

بناءً على مبدأ المشروع الصريح "لا بيانات مزيّفة أبداً، لا تحايل على
قيد حقيقي" — لم يُنشأ `snapchat_adapter.py` لأن أي تطبيق له سيكون إما:
(أ) محولاً لواجهة إعلانات مدفوعة مُقنَّعاً بشكل مضلِّل كـ"نشر عضوي"، أو
(ب) محولاً يرفع NotImplementedError دائماً — كود ميت بلا قيمة حقيقية.
إن ظهر مساراً رسمياً جديداً لاحقاً (شراكة Snap خاصة، أو منتج API جديد)،
يمكن إضافته حينها بنفس عقد PlatformAdapter.
