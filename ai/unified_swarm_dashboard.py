...
def performance_timeline(limit: int = 60) -> List[Dict[str, Any]]:
    ...
    if not cache["expires_at"] or now >= cache["expires_at"]:
        ...
        row: Dict[str, Any] = {"ts": _now_utc.strftime("%H:%M:%S"),
...
def _epoch_from_row(row: Dict[str, Any]) -> Optional[float]:
...
def filter_timeline(rows: List[Dict[str, Any]],
                    range_name: Optional[str] = None,
                    from_iso: Optional[str] = None,
                    to_iso: Optional[str] = None) -> List[Dict[str, Any]]:
    """تصفية سلسلة زمنية بنطاق مخصص.
