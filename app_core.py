...
## التخصص
- تخصصك الأساسي: الإجابة بالعربية ال��صحى الواضحة عن أسئلة المعرفة الإسلامية (قرآن، سنة، فقه، عقيدة) وأسئلة الذكاء الاصطناعي والتقنية.
...
- للإجراءات التي تُغيّر شيئاً خ��رج المحادثة (إرسال، حذف، تعديل)، يطلب المساعد تأكيداً قبل التنفيذ.
...
/* 🆕 حُذفت .nsm-section-title من هنا: كانت نسخة ثانية من "عنوان قسم" بحجم
   ...
   تُستخدم لنفس الغرض بالضبط (عنوان مجم��عة/قسم). كل استخداماتها الأربعة
   ...
   — بدل الإبقاء على ن��طين متنافسين لنفس الوظيفة. */
.nsm-hero-panel {
    position: relative;
    overflow: hidden;
    background: linear-gradient(135deg, var(--gold-soft), var(--emerald-soft)), var(--surface);
...
# هذا العنصر من تيار العناصر (delta stream) يُغيّر مواضع العناصر ا��تالية
...
def apply_runtime_css_and_chrome() -> None:
    ...
    # ── ��رض قسري للون نص التبويبات عبر JS (طبقة حماية أخيرة) ──────────────────
    ...
    (function() {{
        ...
        function nsmBuildPalette() {{
            try {{
                ...
                //      الفردية) ��حتاج للنقر على *كل* الأجداد بالترتيب من
...
def render_skip_link(target_id: str = "nsm-main-content", label: str = "تخطّى إلى المحتوى الرئيسي") -> None:
    ...
    أولاً ضمن ترتيب الـtab، يأخذ المستخدم إلى بداية المحتوى ال��ئيسي بدل
