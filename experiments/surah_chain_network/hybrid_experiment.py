...
المتتالية) تُستخدم كـ"طبقة وسطى مخفية" بين embedding و output head من
نفس معمارية arabic_transformer.py.
...
class SurahChainNetwork:
    """السلسلة الكاملة: 114 طبقة، أبعادها من ملف الإكسل بالضبط."""
...
class HybridExperimentModel:
    """
    embedding (من نفس مشروعك) -> إسقاط لـ7 أبعاد -> سلسلة الـ114 طبقة
    -> إسقاط رجوع لأبعاد المشروع -> output head (من نفس مشروعك)
    """
    def __init__(self, project_d_model=256, vocab_size=VOCAB_SIZE, lr=1e-3):
        ...
        # إسقاطات صغيرة تربط أبعاد المشروع (project_d_model) بأبعاد
        # مدخل/مخرج السلسلة (7) — ضرورية لأن السلسلة معمارية ثابتة من
        # الإكسل مالهاش علاقة بحجم d_model الأصلي للمشروع
