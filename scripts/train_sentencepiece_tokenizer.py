#!/usr/bin/env python3
"""تدريب SentencePiece-style tokenizer على مصادر NSM."""
...
def main() -> int:
    ...
    ap.add_argument("--out", default="models/transformer_ckg_v3/sentencepiece_tokenizer.json")
    args = ap.parse_args()
    ...
    if not texts:
        print("ABORT: لا نصوص")
        return 1
    tok = SentencePieceTokenizer(vocab_size=args.vocab_size)
    n = tok.train(texts, num_merges=args.num_merges)
    os.makedirs(os.path.dirname(args.out) or ".", exist_ok=True)
    tok.save(args.out)
    print(f"✓ SentencePiece vocab={n} merges={len(tok.merges)} → {args.out}")
    s = "الصبر مفتاح الفرج"
    print(f"عينة: {tok.decode(tok.encode(s))!r}")
    return 0
