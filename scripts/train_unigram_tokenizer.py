#!/usr/bin/env python3
"""تدريب Unigram LM tokenizer على مصادر NSM."""
...
def main() -> int:
    ...
    ap.add_argument("--out", default="models/transformer_ckg_v3/unigram_tokenizer.json")
    args = ap.parse_args()
    ...
    if not texts:
        print("ABORT: لا نصوص")
        return 1
    tok = UnigramTokenizer(vocab_size=args.vocab_size)
    n = tok.train(texts, num_iters=args.iters)
    os.makedirs(os.path.dirname(args.out) or ".", exist_ok=True)
    tok.save(args.out)
    print(f"✓ Unigram vocab={n} → {args.out}")
    print(f"عينة: {tok.decode(tok.encode('الصبر مفتاح الفرج'))!r}")
    return 0
